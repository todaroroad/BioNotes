#单细胞可视化自定义函数######
# 来自于文章10.1016/j.jhep.2024.01.034
DotPlot_self=function (object, assay = NULL, features, cols = c("lightgrey", 
                       ...
                       scale = TRUE, scale.by = "radius", scale.min = NA, scale.max = NA) 
{
  ...
  plot <- ggplot(data = data.plot, mapping = aes_string(x = "features.plot", 
    ...
    geom_point(mapping = aes_string(size = "pct.exp"),shape = 21,colour = "black",stroke=0.3)+ 
...
##点图绘制V2######
# 来自于文章10.1016/j.jhep.2024.01.034
DotPlot_self_v2=function (object, assay = NULL, features, cols = c("lightgrey", 
                                                                   "blue"), col.min = -2.5, col.max = 2.5, dot.min = 0, dot.scale = 6, 
                          idents = NULL, group.by = NULL, split.by = NULL, cluster.idents = FALSE, 
                          scale = TRUE, scale.by = "radius", scale.min = NA, scale.max = NA) 
{
  ...
  plot <- ggplot(data = data.plot, mapping = aes_string(x = "features.plot", 
    ...
    # geom_point(mapping = aes_string(size = "pct.exp", color = color.by))+
    # geom_point(mapping = aes_string(size = "pct.exp"),shape = 21,colour = "black",stroke=0.3)+
    geom_point(mapping = aes_string(size = "pct.exp", fill = "avg.exp.scaled"),shape = 21,colour = "black",stroke=0.75)+
    ...
    theme(
      plot.margin = unit(c(1, 1, 1, 1), "cm"),  # 增加外边距 top, right, bottom, and left 
      aspect.ratio = 0.5,
      ...
      axis.text.x = element_text(angle = 45, hjust = 1,vjust = 1,color = "black"),  # 设置x轴刻度标签颜色为黑色
      axis.text.y = element_text(color = "black"),   # 设置y轴刻度标签颜色为黑色
    )+
  ...
  if (!split.colors) {
    plot <- plot + scale_fill_identity(guide = "legend")
...
## main function
StackedVlnPlot_self<- function(obj, features,
                          pt.size = 0, 
                          plot.margin = unit(c(-0.75, 0, -0.75, 0), "cm"),
                          ...) {
